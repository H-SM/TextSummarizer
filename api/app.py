...
from model import generate_summary, generate_summary_context
...
@app.route('/summarycontext', methods=['POST'])
def context_summary_here():
    if not request.json:
        return jsonify({"error": "Request body must be in JSON format"}), 400
    ...
    summary, lenarticle, lensummary = generate_summary_context(context, percentage)
